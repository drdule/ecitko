...
async def get_metrics(db: Database = Depends(get_database)):
    ...
    try:
        ...
        # OCR statistika
        ocr_stats = db.get_ocr_statistics()
        success_rate = round(
            (ocr_stats['successful'] / ocr_stats['total'] * 100) if ocr_stats['total'] > 0 else 0.0,
            2
        )
        ...
        return {
            ...
            "ocr_statistics": {
                "total_tasks": ocr_stats['total'],
                "successful_tasks": ocr_stats['successful'],
                "failed_tasks":  ocr_stats['failed'],
                "error_tasks": ocr_stats['errors'],
                "success_rate_percent": success_rate,
                "average_confidence": ocr_stats['avg_confidence']
            }
        }
    except Exception as e:
